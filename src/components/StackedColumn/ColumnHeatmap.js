...
const ColumnHeatmap = ({ data, keysArray = ["no", "yes"], height, width }) => {
  const svgRef = useRef();

  useEffect(() => {
    if (!data || data.length === 0 || !keysArray.length) return;
    console.log(data);
    const finalData = data[0].data;
    const groupKey = "DST v Actual comparison";

    // Filter out records with empty/null groupKey
    ...
    chart
      ...
      .attr("fill", "#333")
      .text(data[0].title);
    // Draw x-axis (tickFormat blanked since we render our own labels)
    chart
      .append("g")
      .attr("transform", `translate(0,${innerHeight})`)
      .call(d3.axisBottom(x).tickFormat(""));

    // Add wrapped x-axis labels
    ...
    // TOTAL count and percentage above each column
    formattedData.forEach((d) => {
